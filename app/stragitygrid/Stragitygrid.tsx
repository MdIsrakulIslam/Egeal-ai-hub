...
const StrategyGrid: React.FC<StrategyGridProps> = ({ 
  title = "What's Inside the Strategy Grid",
  items = [
    {
  ...
  <div className="relative bg-black rounded-lg p-4 sm:p-6 lg:p-8">
    <h1 className="text-[24px] md:text-[30px] lg:text-[35px] font-semibold leading-[26.25px] text-white text-center">
      {title}
    </h1>
  </div>
</div>
  ...
  <h2 className="text-[16px] font-medium leading-[26.25px] text-white">
    {item.title}
  </h2>
</div>
    ...
    <div className="hidden md:block ml-4 sm:ml-6 lg:ml-10 mt-14">
  {item.details.map((detail, detailIndex) => (
    <p key={detailIndex} className="text-[16px] font-medium leading-[26.25px] text-white sm:mb-2 lg:ml-4">
      {detail}
    </p>
  ))}
</div>
